...
export const updateWalletAsset = async ({ walletid, symbol, amount }) => {
	try {
		const res = await pool.query(
			`INSERT INTO wallet_assets (walletid, symbol, amount, locked) VALUES ($1, $2, $3, $4) ON CONFLICT (walletid, symbol) DO UPDATE SET amount = wallet_assets.amount + $3 RETURNING *`,
			[walletid, symbol, amount, 0]
		)
