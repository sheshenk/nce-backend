...
export const readRealMarketHistory = async (single, { symbol }) => {
	var queryString = 'SELECT time, open, close, high, low, volume FROM real_market_history_btcusd ORDER BY time ASC'
	if (symbol) queryString = `SELECT time, open, close, high, low, volume FROM real_market_history_${symbol} ORDER BY time ASC`
	const res = await pool.query(queryString)
	return res.rows.length ? single ? res.rows[0] : res.rows : null
}
